// Override opsional sebelum config.js load. Nilai akan disaring (path same-origin, https saja untuk link eksternal).
window.__SIPEDAS_ENV__ = window.__SIPEDAS_ENV__ || {};
// ═══════════════════════════════════════════════════════════════════
...
function getDarkMode() {
  try { return localStorage.getItem('_dm') === '1'; } catch(e) { return false; }
}

function setDarkMode(on) {
  try { localStorage.setItem('_dm', on ? '1' : '0'); } catch(e) {}
}
...
function applyDarkMode(on) {
  ...
  document.querySelectorAll('.dm-toggle-btn').forEach(function(btn) {
    btn.classList.toggle('on', on);
  });
  // Update topbar dark mode button
  var dmb = G('dm-btn');
  if (dmb) {
    var ico = dmb.querySelector('i');
    if (ico) ico.className = on ? 'fas fa-sun' : 'fas fa-moon';
    dmb.setAttribute('title', on ? 'Matikan Dark Mode' : 'Aktifkan Dark Mode');
  }
}
...
function toggleDarkMode() {
  ...
  setDarkMode(on);
  applyDarkMode(on);
  toast(on ? 'Mode Gelap aktif 🌙' : 'Mode Terang aktif ☀️', 'inf');
}
...
// ══════════════════════════════════════════
//  PORTAL BACK — Kembali ke menu utama portal
// ══════════════════════════════════════════
function portalBack() {
  // Jika dalam iframe: kirim pesan ke parent
  if (window.self !== window.top) {
    window.parent.postMessage({ action: 'backToMenu', from: 'sipedas' }, '*');
    return;
  }
  // Standalone: kembali ke halaman sebelumnya
  if (document.referrer) {
    window.history.back();
  } else {
    // Fallback: navigasi ke root portal
