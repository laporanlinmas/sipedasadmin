...
// Validasi dilakukan saat runtime di uploadFoto — APPS_SCRIPT_URL bisa jadi alternatif
 ...
 * Misal: "Januari 2024"
 */
function formatBulanTahun(d: Date): string {
  const months = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
  const dJkt = new Date(d.toLocaleString('en-US', { timeZone: TZ }));
 ...
 */
function parseTanggalIndonesia(str?: string): Date | null {
  if (!str) return null;
  const BULAN: Record<string, number> = { januari:1, februari:2, maret:3, april:4, mei:5, juni:6, juli:7, agustus:8, september:9, oktober:10, november:11, desember:12 };
 ...
 */
function parseLaporan(teks: string): ParsedLaporan {
  function ambil(pattern: string, str: string): string {
    const r = new RegExp(pattern, 'is').exec(str);
    return r && r[1] ? r[1].trim() : '';
  }
  let danru = ambil('(Danru\\s*\\d+)', teks);
  if (!danru) {
    const dm = /Danru\s*\d+/i.exec(teks);
    danru = dm ? dm[0].trim() : '';
  }
  let namaDanru = ambil('Danru\\s*\\d+\\s*\\(\\s*(.*?)\\s*\\)', teks);
  if (!namaDanru) {
    namaDanru = ambil('Danru\\s+(?:\\d+\\s*)?([A-Za-z\\s\\.]+)', teks);
  }
  return {
...
function buatKeteranganFoto(source: string, meta: any): string {
...
export async function uploadFoto(data: any): Promise<any> {
  if (!data || !data.foto) throw new Error('Data foto tidak ada.');

  // Guard: minimal salah satu dari FOLDER_UTAMA_ID atau APPS_SCRIPT_URL harus di-set
  const APPS_SCRIPT_URL_CHECK = process.env.APPS_SCRIPT_URL;
  if (!FOLDER_UTAMA_ID && !APPS_SCRIPT_URL_CHECK) {
    throw new Error('[drive-service] Salah satu dari FOLDER_UTAMA_ID atau APPS_SCRIPT_URL harus di-set di environment variables.');
  }
  ...
  const parsed   = teksAsli ? parseLaporan(bersihkanTeks(teksAsli)) : {} as Partial<ParsedLaporan>;

  const waktuNow = new Date();
  const tanggalDate = parseTanggalIndonesia(parsed.tanggal);

  let labelBulan: string, labelTanggal: string;
  if (tanggalDate) {
  ...
  if (APPS_SCRIPT_URL) {
    ...
    if (!FOLDER_UTAMA_ID) {
      throw new Error('FOLDER_UTAMA_ID environment variable is missing.');
    }
  ...
  // Simpan ke sheet Detail Foto (dynamic import untuk hindari circular dependency warning)
  const { appendDetailFoto } = await import('./sheets-service');
...
// ================================================================
//  FETCH FOTO → BASE64  (untuk embed foto di HTML cetak)
// ================================================================

/**
 * Ambil array URL Google Drive, fetch binary-nya di server (tanpa CORS),
 * kembalikan sebagai base64 data-URI.  Maksimal 20 foto, 5 MB per foto.
 */
export async function fetchFotoBase64(urls: string[]): Promise<any> {
  if (!urls || urls.length === 0) return { success: true, data: [] };

  const MAX_BYTES = 5 * 1024 * 1024; // 5 MB per foto
  ...
  for (const rawUrl of urls.slice(0, 20)) {
    try {
      ...
      const m1 = /\/file\/d\/([^\/\?]+)/.exec(rawUrl);
      if (m1) fileId = m1[1];
      const m2 = /[?&]id=([^&]+)/.exec(rawUrl);
      if (!fileId && m2) fileId = m2[1];

      if (!fileId) {
        // Bukan URL Drive — cek whitelist domain sebelum fetch langsung
        // untuk mencegah SSRF (Server Side Request Forgery)
        ...
        if (!ALLOWED_FETCH_DOMAINS.includes(urlHostname)) {
          console.warn('[fetchFotoBase64] domain tidak diizinkan:', urlHostname);
          results.push('');
          continue;
        }
        ...
        if (buf.byteLength > MAX_BYTES) { results.push(''); continue; }
        const mime = res.headers.get('content-type') || 'image/jpeg';
        results.push(`data:${mime};base64,${Buffer.from(buf).toString('base64')}`);
        continue;
      }

      // Gunakan Google Drive API untuk download (auth service account)
      const drive = await getDrive();
      ...
      // Deteksi mime dari header atau fallback ke jpeg
      const mimeHeader = (fileRes.headers?.['content-type'] as string) || 'image/jpeg';
      const mime = mimeHeader.split(';')[0].trim() || 'image/jpeg';
      results.push(`data:${mime};base64,${buf.toString('base64')}`);

    } catch (e: any) {
      console.error('[fetchFotoBase64] error for url:', rawUrl, e.message);
      results.push(''); // kosong = foto gagal
    }
  }
...
// ================================================================
//  UPLOAD DOCX → GOOGLE DRIVE (buka sebagai Google Docs)
// ================================================================

/**
 * Upload file DOCX (base64) ke Google Drive di folder laporan,
 * lalu kembalikan URL untuk dibuka/diedit di Google Docs.
 * Menyimpan di folder yang sama dengan foto laporan (FOLDER_UTAMA_ID).
 */
export async function uploadHtmlToGoogleDocs(
  html: string,
  title: string,
  docxBase64?: string
): Promise<any> {
  try {
    const drive = await getDrive();
    const parents = FOLDER_UTAMA_ID ? [FOLDER_UTAMA_ID] : undefined;

    let fileId: string;
    let mimeUpload: string;
    let bodyBuffer: Buffer;

    if (docxBase64) {
      // Upload DOCX — Drive bisa membukanya langsung di Google Docs viewer/editor
      bodyBuffer = Buffer.from(docxBase64, 'base64');
      mimeUpload = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
    } else {
      // Fallback: upload HTML, konversi ke Google Docs (bisa kena quota)
      bodyBuffer = Buffer.from(html || '', 'utf-8');
      mimeUpload = 'text/html';
    }

    const { Readable: RS } = await import('stream');

    const createOpts: any = {
      requestBody: {
        name: title || 'Laporan Patroli',
        // Jika DOCX: mimeType tujuan = Google Docs agar bisa diedit
        // Jika HTML: sama
      ...
      media: {
        mimeType: mimeUpload,
        body: RS.from(bodyBuffer),
      },
      fields: 'id, webViewLink',
      supportsAllDrives: true,
    };

    const fileRes = await drive.files.create(createOpts);
    fileId = fileRes.data.id;
    const webViewLink = fileRes.data.webViewLink;

    // Beri akses anyone with link = reader (bukan writer — dokumen resmi tidak boleh diedit sembarang orang)
    await drive.permissions.create({
      fileId,
    ...
    return {
      ...
      data: {
        fileId,
        url: webViewLink || `https://docs.google.com/document/d/${fileId}/edit`,
      },
    };
  } catch (e: any) {
    console.error('[uploadHtmlToGoogleDocs] error:', e.message);

    // Jika quota terlampaui, gunakan alternatif: buka Google Docs editor kosong
    // dengan instruksi copy-paste (tidak ideal tapi tidak crash)
    if (e.message?.includes('quota') || e.message?.includes('storage')) {
      return {
        success: false,
        message: 'Drive storage penuh. Gunakan Download DOCX lalu upload manual ke Google Drive.',
        quotaError: true,
      };
    }
    return { success: false, message: 'Gagal upload ke Google Docs: ' + e.message };
  }
}
